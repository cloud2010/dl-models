...
if __name__ == "__main__":
    ...
    y_pred_list = [cross_val_predict(
        clf[args.classifier], X[:, 0:i+1], y, cv=args.kfolds, n_jobs=-1) for i in tgrange(0, X.shape[1])]
    ...
    acc_clf = [accuracy_score(y, y_pred_list[i]) for i in tgrange(0, X.shape[1])]
    ...
    y_pred_list_fs = [cross_val_predict(
        clf[args.classifier], X[:, fs_idxs[0:i+1]], y, cv=args.kfolds, n_jobs=-1) for i in tgrange(0, X.shape[1])]
