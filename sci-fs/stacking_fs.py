...
if __name__ == "__main__":
    ...
    y_pred_list = [cross_val_predict(
        clf, X[:, 0:i+1], y, cv=args.kfolds, n_jobs=-1) for i in range(0, X.shape[1])]
    ...
    recall_pre_f1_clf = [precision_recall_fscore_support(
        y, y_pred_list[i], average='binary') for i in range(0, X.shape[1])]
    ...
    y_pred_list_fs = [cross_val_predict(
        clf, X[:, fs_idxs[0:i+1]], y, cv=args.kfolds, n_jobs=-1) for i in range(0, X.shape[1])]
    ...
    recall_pre_f1_clf_fs = [precision_recall_fscore_support(
        y, y_pred_list_fs[i], average='binary') for i in range(0, X.shape[1])]
