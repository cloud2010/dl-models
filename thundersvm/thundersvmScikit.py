...
class SvmModel(ThundersvmBase):
    ...
    def fit(self, X, y):
        ...
        self.dual_coef_ = np.array([dual_coef[index] for index in range(
            0, (self.n_classes - 1) * self.n_sv)]).astype(float)
        ...
        self.n_support_ = np.array([n_support_[index]
                                    for index in range(0, self.n_classes)]).astype(int)
    ...
    def _dense_predict(self, X):
        ...
        self.predict_label = np.array([self.predict_label_ptr[index]
    ...
    def _dense_decision_function(self, X):
        ...
        self.dec_values = np.array([dec_value_ptr[index]
                                    for index in range(0, dec_size)]).astype(float)
